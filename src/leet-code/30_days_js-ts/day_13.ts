...
// new functions when old promises resolve. promisePool should execute functions[i]
// then functions[i + 1] then functions[i + 2], etc. When the last promise resolves, promisePool should also resolve.
//
...
async function promisePool(functions: F[], n: number): Promise<any> {
    ...
    const promises = Array(n).fill(0).map(evaluateNext);
