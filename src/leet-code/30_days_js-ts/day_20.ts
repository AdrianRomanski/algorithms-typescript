...
function objDiff(obj1: any, obj2: any): any {
    function isNotObject(obj: any) {
     return typeof obj !== 'object';
    }
    if (obj1 === obj2) {
        return {};
    }
    if (!obj1 || !obj2 || isNotObject(obj1) || isNotObject(obj2) || Array.isArray(obj1) !== Array.isArray(obj2)) {
        return [obj1, obj2];
    }
    const result = {};
    for (const key in obj1) {
        if (key in obj2) {
            // if key is present in another object we check for difference
            const difference = objDiff(obj1[key], obj2[key]);
            // if we got difference, we append it to result
